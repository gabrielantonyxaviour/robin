...
task("deploy-verifier", "Deploys the RobinXWorldIdVerifier contract")
  .addOptionalParam(
    "verify",
  ...
  .setAction(async (taskArgs) => {
    console.log(`Deploying RobinXWorldIdVerifier contract to ${network.name}`);
    ...
    console.log(
      `\nWaiting ${
        ...
        verifierContract.deployTransaction.hash
      } to be confirmed...`
    );
    ...
    console.log(
      "\nDeployed RobinXWorldIdVerifier contract to:",
    ...
    console.log(
      `\n RobinXWorldIdVerifier contract deployed to ${verifierContract.address} on ${network.name}`
    );
  });
